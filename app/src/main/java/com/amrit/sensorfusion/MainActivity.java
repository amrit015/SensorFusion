package com.amrit.sensorfusion;
...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        seriesPitch = new LineGraphSeries<DataPoint>();
        seriesRoll = new LineGraphSeries<DataPoint>();
        seriesYaw = new LineGraphSeries<DataPoint>();
        ...
        qPitch = (TextView) findViewById(R.id.pitch_qua);
        qRoll = (TextView) findViewById(R.id.roll_qua);
        qYaw = (TextView) findViewById(R.id.yaw_qua);
...
//                float mYaw = mOrientation[0]; // orientation contains: yaw, pitch and roll
//                float mPitch = mOrientation[1];
//                float mRoll = mOrientation[2];

                SensorManager.getQuaternionFromVector(mQuaternion, mOrientation);

                mYaw = mQuaternion[1]; // orientation contains: azimut, displayPitch and displayRoll
                mPitch = mQuaternion[2];
                mRoll = mQuaternion[3];

                // calibrating the quaternion values by taking difference from the last value
    ...
    private void updateValues() {
        if (!displayPitch.equals("") && !displayRoll.equals("") && !displayYaw.equals("")) {
            ...
            qPitch.setText(d.format(newPitchOutQ) + " rad/s");
    ...
    private class calculateFusedOrientationTask extends TimerTask {
        float filter_coefficient = 0.90f;
        ...
        public void run() {
            ...
            // calibrating the sensor values by taking difference from the last value
